...
class AgeGenderHelper:
	def __init__(self, config):
		# store the config object and build the age bins used
		# for constructing class labels
		self.config = config
		self.ageBins = self.buildAgeBins()

	def buildAgeBins(self):
		# init the list of age bins based on the Adience-dataset
	...
	def toLabel(self, age, gender):
		...
		# assume we are determining the gender label
	...
	def buildOneOffMappings(self, le):
		# sort the labels in ascending order (according to age)
		# init the one-off mappings for computing accuracy
	...
	def buildPathsAndLabels(self):
		# init the list of image paths and labels
		paths = []
		labels = []

		# grab the paths to the text files
		textPaths = os.path.sep.join([self.config.LABELS_PATH,
			"*.txt"])
		textPaths = glob.glob(textPaths)

		# loop over the folds paths
		for textPath in textPaths:
			# load the contents of the text file, skipping the
			# header
			rows = open(textPath).read()
	...
	def visualizeAge(agePreds, le):
		# init the canvas and sort the predictions according
